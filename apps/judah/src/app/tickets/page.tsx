...
import { lines, loadPageData } from "@/lib/page-data";
  ...
  return (
    <div className="tickets-page">
      <div className="checkout">
        ...
        {ticketing.mode !== "onSite" ? (
          ...
          <div>
            ...
            {ticketing.outlets.length > 0 ? (
              <ul className="checkout-tiers">
                {ticketing.outlets.map((outlet) => (
                  <li key={outlet.href} className="checkout-tier">
                    ...
                    <a href={outlet.href} className="btn-primary">
                      {c.tickets.ctaLabel}
                    </a>
                  </li>
                ))}
              ...
              <a href={ticketing.href} className="btn-primary">
                {c.tickets.ctaLabel}
              </a>
            ) : null}
          </div>
        ...
        {perks.length > 0 ? (
          <div style={{ marginTop: "clamp(40px,6cqi,80px)" }}>
            ...
            <ul className="checkout-tiers">
              {perks.map((perk, i) => (
                <li key={perk} className="checkout-tier">
                  ...
                  <span className="checkout-note">{String(i + 1).padStart(2, "0")}</span>
                </li>
              ))}
            </ul>
          </div>
        ...
        {c.tickets.note ? (
          <p className="checkout-note" style={{ marginTop: "clamp(30px,4cqi,50px)" }}>
            {c.tickets.note}
          </p>
        ) : null}

        {c.tickets.phone ? (
          <p className="checkout-note" style={{ marginTop: "20px" }}>
            Ticket questions{" "}
