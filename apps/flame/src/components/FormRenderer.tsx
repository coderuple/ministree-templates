...
export default function FormRenderer({ slug, fields }: { slug: string; fields: FormField[] }) {
  ...
  if (state === "done") {
    return (
      <div className="rounded-flame border border-line bg-surface p-8 text-center">
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-5">
      {fields.map((f, i) => {
        ...
        const common = "w-full rounded-flame border border-line bg-bg px-4 py-2.5 text-ink outline-none focus:border-ember";
        return (
          <div key={name}>
      ...
      <button
        ...
        className="inline-flex items-center justify-center rounded-full bg-ember px-6 py-2.5 text-sm font-medium text-bg transition-opacity hover:opacity-90 disabled:opacity-60"
      >
