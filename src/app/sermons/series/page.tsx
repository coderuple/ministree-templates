      ...
      <Container className="py-12">
        {items.length ? (
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {items.map((s) => (
              <Link key={s.id} href={`${base}/${s.slug}`} className="group overflow-hidden rounded-2xl border border-border bg-bg transition-colors hover:border-accent/50">
                <div className="aspect-[16/9] bg-surface">
                  {s.coverImageUrl ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={s.coverImageUrl} alt={s.name} className="h-full w-full object-cover" />
                  ) : (
                    <div className="h-full w-full bg-gradient-to-br from-accent/15 to-accent-2/10" />
                  )}
                </div>
                <div className="p-5">
                  <h3 className="font-display text-xl font-semibold group-hover:text-accent">{s.name}</h3>
